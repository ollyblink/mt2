...
public class JobSubmissionExecutor extends AbstractExecutor {
	...
	public static String classId = IDCreator.INSTANCE.createTimeRandomID(JobSubmissionExecutor.class.getSimpleName());
	...
	public void submit(final Job job) {
		...
		JobProcedureDomain outputJPD = JobProcedureDomain.create(job.id(), job.submissionCount(), classId, procedure.executable().getClass().getSimpleName(), procedure.procedureIndex(),
				procedure.currentExecutionNumber());
		procedure.dataInputDomain(JobProcedureDomain.create(job.id(), job.submissionCount(), classId, DomainProvider.INITIAL_PROCEDURE, -1, 0)
				.expectedNrOfFiles(estimatedNrOfFiles(keysFilePaths, job.maxFileSize().value()))).addOutputDomain(outputJPD);
